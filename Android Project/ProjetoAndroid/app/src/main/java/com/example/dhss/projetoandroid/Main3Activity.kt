...
class Main3Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var buttonNext = findViewById<Button>(R.id.button3)

        val recife:Double = intent.getDoubleExtra("temperatura" , 0.0)

        textViewCidade = findViewById<TextView>(R.id.textViewCidade)
        textViewTemperatura = findViewById<TextView>(R.id.textViewTemp)

        Thread{
            var cidade = CidadeClimaServiceRecife.getClimaCidade()
            runOnUiThread {
        ...
        buttonNext.setOnClickListener {
            ...
            intent.putExtra("temperatura", gravata)
